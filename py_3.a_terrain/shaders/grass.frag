...
vec3 point_light(vec3 N, vec3 V, PointLight light, vec3 F0) {
  // Direction vector
  const vec3 D = normalize(light.position - fs_in.frag_pos);
  const vec3 H = normalize(V + D);

  // Attenuation
  const float distance = length(light.position - fs_in.frag_pos);
...
vec3 light_colors(vec3 tex_color) {
  const vec3 V = normalize(cam_pos - fs_in.frag_pos);

  // Precompute the surface response at normal incidence
  const vec3 F0 = mix(vec3(0.04), material.a, material.s);

  // Directional lights
  vec3 Lo = directional_light(fs_in.normal, V, global_light, F0);

  if (local_light_blend > 0.0) {
    for (int i = 0; i < max_lights; i++) {
      Lo += point_light(fs_in.normal, V, lights[i], F0);
      if (i == num_lights) {
        break;
      }
    }
  ...
  Lo += spot_light(fs_in.normal, V, flash_light, F0);
...
void main() {
  const vec4 color_full = texture(u_tex_albedo, fs_in.uv_0);
