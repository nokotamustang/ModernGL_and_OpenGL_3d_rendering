...
def compute_tangents(vertices, indices, texture_coords, texture_indices):
    """
    Compute tangent vectors for a mesh using MikkTSpace-like algorithm.
    Args:
        vertices: List of (x, y, z) vertex positions.
        indices: List of (v0, v1, v2) triangle indices for vertices.
        texture_coords: List of (u, v) UV coordinates.
        texture_indices: List of (t0, t1, t2) triangle indices for UVs.
    Returns:
        tangents: List of (tx, ty, tz) tangent vectors per vertex.
    """
    tangents = []
    for i in range(0, len(indices), 2):
        triangle_indices = indices[i:i+2]
        triangle_texture_indices = texture_indices[i:i+2]

        # Triangle 1
        point_1 = vertices[triangle_indices[0][0]]
        point_2 = vertices[triangle_indices[0][1]]
        point_3 = vertices[triangle_indices[0][2]]
        tc_1 = texture_coords[triangle_texture_indices[0][0]]
        tc_2 = texture_coords[triangle_texture_indices[0][1]]
        tc_3 = texture_coords[triangle_texture_indices[0][2]]
        triangle_edge_1 = (point_2[0] - point_1[0], point_2[1] - point_1[1], point_2[2] - point_1[2])
        triangle_edge_2 = (point_3[0] - point_1[0], point_3[1] - point_1[1], point_3[2] - point_1[2])
        delta_uv_1 = (tc_2[0] - tc_1[0], tc_2[1] - tc_1[1])
        delta_uv_2 = (tc_3[0] - tc_1[0], tc_3[1] - tc_1[1])
        f = 1.0 / (delta_uv_1[0] * delta_uv_2[1] - delta_uv_2[0] * delta_uv_1[1])
        tangent_1_x = f * (delta_uv_2[1] * triangle_edge_1[0] - delta_uv_1[1] * triangle_edge_2[0])
        tangent_1_y = f * (delta_uv_2[1] * triangle_edge_1[1] - delta_uv_1[1] * triangle_edge_2[1])
        tangent_1_z = f * (delta_uv_2[1] * triangle_edge_1[2] - delta_uv_1[1] * triangle_edge_2[2])

        # Triangle 2
        point_1 = vertices[triangle_indices[1][0]]
        point_2 = vertices[triangle_indices[1][1]]
        point_3 = vertices[triangle_indices[1][2]]
        tc_1 = texture_coords[triangle_texture_indices[1][0]]
        tc_2 = texture_coords[triangle_texture_indices[1][1]]
        tc_3 = texture_coords[triangle_texture_indices[1][2]]
        triangle_edge_1 = (point_2[0] - point_1[0], point_2[1] - point_1[1], point_2[2] - point_1[2])
        triangle_edge_2 = (point_3[0] - point_1[0], point_3[1] - point_1[1], point_3[2] - point_1[2])
        delta_uv_1 = (tc_2[0] - tc_1[0], tc_2[1] - tc_1[1])
        delta_uv_2 = (tc_3[0] - tc_1[0], tc_3[1] - tc_1[1])
        f = 1.0 / (delta_uv_1[0] * delta_uv_2[1] - delta_uv_2[0] * delta_uv_1[1])
        tangent_2_x = f * (delta_uv_2[1] * triangle_edge_1[0] - delta_uv_1[1] * triangle_edge_2[0])
        tangent_2_y = f * (delta_uv_2[1] * triangle_edge_1[1] - delta_uv_1[1] * triangle_edge_2[1])
        tangent_2_z = f * (delta_uv_2[1] * triangle_edge_1[2] - delta_uv_1[1] * triangle_edge_2[2])

        # Tangents for each vertex of each triangle
        tangents.append((tangent_1_x, tangent_1_y, tangent_1_z, tangent_1_x, tangent_1_y, tangent_1_z, tangent_1_x, tangent_1_y, tangent_1_z,
                         tangent_2_x, tangent_2_y, tangent_2_z, tangent_2_x, tangent_2_y, tangent_2_z, tangent_2_x, tangent_2_y, tangent_2_z))
    return tangents
...
class Texture:
    ...
    def get_depth_texture(self, size, name='depth_texture', disable_repeat=False):
    ...
    def get_color_texture(self, size, name='color_texture', samples=None, disable_repeat=False):
        if name in self.texture_map:
            return self.texture_map[name]
        color_texture = self.ctx.texture(size=size, components=4, samples=samples)
        # Remove repetition
        if disable_repeat:
...
class Shadow():
    def __init__(self, app, name="depth_texture", depth_size=[4096, 4096]):
        ...
        self.depth_tex_id = self.app.texture.get_depth_texture(depth_size, name, disable_repeat=True)
...
class PrototypeCube:
    def __init__(self, app,
        ...
        self.shadow_vbo = self.get_shadow_vbo()
        ...
        self.shadow_vao = self.get_shadow_vao(self.shadow_vbo, self.shadow_program)
    ...
    def get_vao(self, vbo, shader_program):
        vao = self.ctx.vertex_array(shader_program, [
            (vbo, '2f 3f 3f 3f', 'in_texcoord_0', 'in_position', 'in_normal', 'in_tangent'),
        ])
        return vao
    ...
    def get_vertex_data(self, all_data: bool = True):
        ...
        # Tangents
        tangents = compute_tangents(vertices, indices, texture_coords, texture_indices)
        tangents = numpy.array(tangents, dtype='f4').reshape(int(len(tangents * 6)), 3)
        vertex_data = numpy.hstack([vertex_data, tangents])
    ...
    def get_shadow_vbo(self):
        return self.ctx.buffer(self.get_vertex_data(all_data=False))
...
class Cube:
    def __init__(self, app, albedo=(1.0, 1.0, 1.0), roughness=0.75, metallic=0.25,
        ...
        if bump:
            self.tex_bump_id = app.texture.get_texture(path=f'../textures/{bump}.png')
        else:
            self.tex_bump_id = -1
        if parallax:
    ...
    def render(self):
        # Texture
        self.shader_program['u_tex_albedo'] = self.tex_id
        self.app.texture.textures[self.tex_id].use(location=self.tex_id)
        if self.tex_bump_id > -1:
            self.shader_program['bump_mix'] = 1.0
            self.shader_program['u_tex_bump'] = self.tex_bump_id
            self.app.texture.textures[self.tex_bump_id].use(location=self.tex_bump_id)
        else:
        ...
        if self.tex_parallax_id > -1:
            ...
            self.shader_program['parallax_mix'] = 0.0
        # Position
...
class Floor(Cube):
    def __init__(self, app, albedo=(1.0, 1.0, 1.0), roughness=0.9, metallic=0.2,
                 ...
                 bump: str = None, parallax: str = None, parallax_scale: float = None,
                 can_update=False):
        super().__init__(app, albedo, roughness, metallic, position,
                         scale, texture, name="floor", bump=bump, parallax=parallax,
                         parallax_scale=parallax_scale, can_update=can_update)
        ...
        for i in range(-_n, _n):
            for j in range(-_n, _n):
                self.objects.append(Floor(app, position=(i*_s*2.0, _h, j*_s*2.0), scale=(_s, 0.1, _s),
                                          ...
                                          parallax="stone_floor_disp_depth_rgb", parallax_scale=0.025))
