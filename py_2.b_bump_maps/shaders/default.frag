...
vec3 light_colors(vec3 tex_color, vec3 bump_xyz) {

  // The classic surface normal and the bump mapped TBN multiplied one
  // const vec3 N = normal;
  // const vec3 N = normalize(bump_t_b_n * bump_xyz);

  // Since I've leaving the bump map optional, we flip the state with bump_mix as 1.0 or 0.0
  // const vec3 N = (bump_mix == 1.0) ? normalize(bump_t_b_n * bump_xyz) : normal;

  // Written as optimized branch-free, if no switch is needed obviously it would be better:
  const vec3 N = mix(normal, normalize(bump_t_b_n * bump_xyz), bump_mix);
...
void main() {
  const vec3 bump = texture(u_tex_bump, uv_0).rgb * 2.0 - 1.0; // Transform from [0..1] to [-1..1] range

  vec3 color = texture(u_tex_albedo, uv_0).rgb;
  color = pow(color, gamma);
  color = light_colors(color, bump);
